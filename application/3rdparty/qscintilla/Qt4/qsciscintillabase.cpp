...
// ### sintegrial: direct style setting without notification
void QsciScintillaBase::DirectSetStyle(int length, char style)
{
  sci->pdoc->SetStyleForSilent(length, style);
}

// ### sintegrial: direct style start
void QsciScintillaBase::DirectStartStyling(int position, char mask)
{
  sci->pdoc->StartStyling(position, mask);
}

// ### sintegrial: direct character value
char QsciScintillaBase::DirectCharAt(int position) const
{
  return sci->pdoc->CharAt(position);
}
