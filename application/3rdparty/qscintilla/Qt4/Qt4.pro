...
INCLUDEPATH = . \
    ../include \
    ../src
DEFINES = QT \ # QSCINTILLA_MAKE_DLL \
    SCI_LEXER

# Handle both Qt v4 and v3.
target.path = $$[QT_INSTALL_LIBS]
isEmpty(target.path):target.path = $(QTDIR)/lib
...
isEmpty(trans.path):trans.path = $(QTDIR)/translations
qsci.path = $$[QT_INSTALL_DATA]
qsci.files = ../qsci
isEmpty(qsci.path):qsci.path = $(QTDIR)
INSTALLS += header \
    trans \
    qsci \
    target
