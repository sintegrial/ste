...
static inline void Platform_MakeUpperW(wchar_t* wstr, unsigned int len)
{
   ...
   // qDebug() << QString::fromWCharArray(wstr, len) << " " << len;
}
...
// ### sintegrial: simplified version without notification
bool Document::SetStyleForSilent(int length, char style) {
  if (enteredStyling != 0) {
    return false;
  } else {
    enteredStyling++;
    style &= stylingMask;
    int prevEndStyled = endStyled;
    cb.SetStyleFor(endStyled, length, style, stylingMask);
    endStyled += length;
    enteredStyling--;
    return true;
  }
